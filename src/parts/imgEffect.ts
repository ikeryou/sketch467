...
export class ImgEffect extends Canvas {
  ...
  protected _update(): void {
    ...
    this._imgs.forEach((m:any, i:number) => {
      ...
      m.position.x = ix * size - (size * this._line * 0.5) + size * 0
      m.position.y = iy * size - (size * this._line * 0.5) + size * 0

      // m.position.x += offsetX * (iy % 2 == 0 ? 1 : -1)
    })

    const mouse = new Vector2(Util.map(MousePointer.instance.easeNormal.x, -1, 1, -1, 1), -1 * Util.map(MousePointer.instance.easeNormal.y, -1, 1, -1, 1))
